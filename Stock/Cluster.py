...
def fe_stock_and_time(df_bucket_all, data_type):
    ...
    if data_type == 'test':
        df_stock = pd.read_parquet(f'{my_input_path}/df_stock_feature.parquet')
...
df_train = pd.read_parquet(os.path.join(my_input_path, 'df_feature.parquet'))
print(f'df_train shape: {df_train.shape}')

df_test = fe_all_stock('test')
print('test data feature engineering finish')
print(f'df_test shape: {df_test.shape}')


def kmeans_cluster(df_feature, n_cluster):
    # process train_data
    cluster_features = ['wap1_mean', 'log_return1_realized_volatility']
    stock_agg_features = {
        'log_return1_realized_volatility': ['mean', 'std']
    }

    df_cluster = df_feature.groupby('stock_id')[cluster_features].agg(stock_agg_features).reset_index()
    df_cluster.columns = ['_'.join(i) for i in df_cluster.columns]
    df_cluster.rename(columns={'stock_id_': 'stock_id'}, inplace=True)

    print(f'df_cluster shape: {df_cluster.shape}')
    df_cluster.head()

    df_id = df_cluster['stock_id']
    df_cluster.drop(['stock_id'], axis=1, inplace=True)
    ...
    #         CH_score.append(metrics.calinski_harabasz_score(x, y_pred))

    #     plt.figure()
    #     plt.plot(Nlist, CH_score, marker='o')
    #     plt.show()

    #     max_value = max(CH_score)
    #     print(f'max CH_score: {max_value}, n_cluster={CH_score.index(max_value)+2}')

    model = KMeans(n_clusters=n_cluster, init='k-means++')
    model.fit(x)
    y_pred = model.predict(x)

    #     joblib.dump(model, f'{model_path}/cluster_model.pkl')
    #     model = joblib.load(f'{model_path}/cluster_model.pkl')

    #     color = ['r','g','b','y']

    #     plt.figure()
    #     for i in range(x.shape[0]):
    #         plt.scatter(x[i, 0], x[i, 1], c=color[y_pred[i]])
    #     plt.show()

    clus_result = pd.DataFrame(data=y_pred, columns=['cluster'], dtype='category')
    clus_result = pd.concat([df_id, clus_result], axis=1)

    return clus_result


df_cluster = kmeans_cluster(df_train, n_cluster=num_clus)

df_train = pd.merge(df_train, df_cluster, how='left', on='stock_id')
print(f'df_train shape: {df_train.shape}')

df_test = pd.merge(df_test, df_cluster, how='left', on='stock_id')
print(f'df_test shape: {df_test.shape}')
...
def feval_rmspe(y_pred, train_set):
    y_true = train_set.get_label()
    loss = rmspe(y_true, y_pred)
...
def fit_model(X_train, y_train):
    ...
    # feature importance fi_df用于看特征的权重
    fi_df = pd.DataFrame()
    ...
    for fold, (train_index, valid_index) in tqdm(enumerate(kfold)):
        # split
        ...
        train_weights = 1 / np.square(y_tr)
        val_weights = 1 / np.square(y_val)

        train_set = lgb.Dataset(X_tr, y_tr, weight=train_weights, categorical_feature=['stock_id'])
        val_set = lgb.Dataset(X_val, y_val, weight=val_weights, reference=train_set, categorical_feature=['stock_id'])
        ...
        fi_df[f'importance_fold{fold}'] = model.feature_importance(importance_type="gain")
        fi_df['importance'] += fi_df[f'importance_fold{fold}'].values

        # save model
        joblib.dump(model, f'{model_path}/reg_model_fold{fold}.pkl')
...
def train_and_eval(df_train, df_test):
    drops = ['time_id', 'row_id', 'target', 'cluster']
    features = [f for f in df_train.columns.values.tolist() if f not in drops]

    models, fi_df = fit_model(df_train[features], df_train['target'])
    ...
    return df_test[['row_id', 'target']]


# 在已有训练好的模型的情况下，光测试（模型从文件读取）
def test(df_test):
    drops = ['time_id', 'row_id', 'target', 'cluster']
    features = [f for f in df_test.columns.values.tolist() if f not in drops]

    y_pred = np.zeros(df_test.shape[0])
    files = glob.glob(f'{model_path}/reg_model_*.pkl')
...
# sub = train_and_eval(df_train, df_test)
# sub.to_csv('submission.csv', index=False)
# sub

sub = pd.DataFrame()
for n in list(range(0, num_clus)):

    df_train_clus = df_train[df_train['cluster'] == n]
    df_train_clus = df_train_clus.reset_index()
    df_train_clus.drop(['index'], axis=1, inplace=True)

    df_test_clus = df_test[df_test['cluster'] == n]
    df_test_clus = df_test_clus.reset_index()
    df_test_clus.drop(['index'], axis=1, inplace=True)

    if df_test_clus.shape[0] > 0:
        sub_clus = train_and_eval(df_train_clus, df_test_clus)
        sub = pd.concat([sub, sub_clus], axis=0)
